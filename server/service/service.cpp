...
serviceFunc Service::getHandler(int id) {
  ...
  if (it == serviceMapping_.end()) {
    return
        [=](const TcpConnectionPtr &ptr, Json::Value &json, muduo::Timestamp) {
...
Service::Service() {
  ...
  serviceMapping_.insert(
      {REQ_CODE::REG_CODE_REG, std::bind(&Service::reg, this, _1, _2, _3)});
  serviceMapping_.insert(
      {REQ_CODE::REG_CODE_LOGIN, std::bind(&Service::login, this, _1, _2, _3)});
  serviceMapping_.insert({REQ_CODE::REG_CODE_ADDFRIEDN,
                          std::bind(&Service::addFriend, this, _1, _2, _3)});
  serviceMapping_.insert(
      {REQ_CODE::REG_CODE_GETOFFLINEMSG,
       std::bind(&Service::getOfflienMsg, this, _1, _2, _3)});
  serviceMapping_.insert({REQ_CODE::REG_CODE_ACCEPTFRIEND,
                          std::bind(&Service::acceptFriend, this, _1, _2, _3)});
  serviceMapping_.insert(
      {REQ_CODE::REG_CODE_GETFRIENDLIST,
       std::bind(&Service::getFriendList, this, _1, _2, _3)});
}

void Service::reg(const TcpConnectionPtr &ptr, Json::Value &json,
                  muduo::Timestamp) {
  LOG_INFO << "\nregister";
  User user;

  // 参数校验
  if (json["account"].isNull() || json["password"].isNull() ||
      json["account"].asString() == "" || json["password"].asString() == "")
...
void Service::login(const TcpConnectionPtr &ptr, Json::Value &json,
                    muduo::Timestamp) {
  LOG_INFO << "\nlogin";
  User user;
  Json::Value res;
...
void Service::addFriend(const TcpConnectionPtr &ptr, Json::Value &json,
                        muduo::Timestamp) {
  LOG_INFO << "\naddFriend";
  User user;
  Friend fd;
  // 参数判断
  if (json["id"].isNull() || !json["id"].isInt() || json["account"].isNull() ||
      json["account"].asString() == "") {
    sendResponse(ptr, RES_CODE::RES_CODE_FAILE, "incorrect parameter");
    return;
  }

  if (!userIsOnline(json["id"].asUInt(), ptr))
    return;
  ...
  if (friendmodel_.isMyfriend(fd)) {
    sendResponse(ptr, RES_CODE::RES_CODE_FAILE,
                 "The other person is already your friend");
    return;
  }
  ...
  if (user.isOnline()) {
    ...
    if (!json["content"].isNull() && json["content"].asString() != "")
      msg["content"] = json["content"];
    std::unordered_map<int, TcpConnectionPtr>::iterator it;
    bool isonline = false;
    {
      std::lock_guard<std::mutex> guard(mutex_);
      it = conns_.find(user.getId());
      if (it != conns_.end())
        isonline = true;
    }
    if (isonline)
      it->second->send(jswriter.write(msg));
  } else {
    Offlinemsg offline;
...
void Service::sendResponse(const TcpConnectionPtr &ptr, int rescode,
                           const char *content) {
  Json::Value res;
...
void Service::getOfflienMsg(const TcpConnectionPtr &ptr, Json::Value &json,
                            muduo::Timestamp) {
  LOG_INFO << "\n"
           << "getOfflineMsg";
  // 参数验证
...
void Service::acceptFriend(const TcpConnectionPtr &ptr, Json::Value &json,
                           muduo::Timestamp) {
  LOG_INFO << "\n"
           << "acceptFriend";
  Friend f;
  // 参数验证
  if (json["fromid"].isNull() || json["toid"].isNull() ||
      json["agree"].isNull() || !json["fromid"].isInt() ||
      !json["toid"].isInt() || !json["agree"].isInt()) {
    sendResponse(ptr, RES_CODE::RES_CODE_FAILE, "incorrect parameter");
    return;
  }

  if (!userIsOnline(json["toid"].asUInt(), ptr))
    return;

  //判断是否已是好友
  f.myid_ = json["toid"].asUInt();
  f.friendid_ = json["fromid"].asUInt();
  if(friendmodel_.isMyfriend(f))  return; 

  // 添加好友
  if (json["agree"].asBool()) {
    f.remarks_ = json["remarks"].asString();
    if (friendmodel_.insert(f))
      sendResponse(ptr, RES_CODE::RES_CODE_OK, "");
    else
      sendResponse(ptr, RES_CODE::RES_CODE_FAILE, "fail");
    return;
  }
  sendResponse(ptr, RES_CODE::RES_CODE_OK, "");
}
...
void Service::getFriendList(const TcpConnectionPtr &ptr, Json::Value &json,
                            muduo::Timestamp) {
  LOG_INFO << "\ngetFriendList";
  unsigned int id;
  User user;
...
void Service::sendMsgToFriend(const TcpConnectionPtr &ptr, Json::Value &json,
                              muduo::Timestamp) {
  User user;
  Offlinemsg offmsg;
  // 参数验证
  if (!json["fromid"].isInt() || !json["toid"].isInt() ||
      !json["content"].isString() || json["content"].asString() == "") {
    sendResponse(ptr, RES_CODE::RES_CODE_FAILE, "incorrect parameter");
    return;
  }
  if (!userIsOnline(json["fromid"].asUInt(), ptr))
    return;

  // 对方在线
  if (userModel_.selectUserById(user, json["toid"].asUInt()) &&
      user.isOnline()) {
    Json::Value msg;
    msg["fromid"] = json["fromid"].asUInt();
    msg["msg_type"] = 0;
    ...
    // 写入离线消息列表
    offmsg.fromid_ = json["fromid"].asUInt();
    offmsg.toid_ = user.getId();
    offmsg.msg_type_ = 0 ;
    offmsg.content_  = json["content"].asString();
    if(!offlinemsgmodel_.insert(offmsg)) sendResponse(ptr,RES_CODE::RES_CODE_FAILE,"send fail");
  }
  sendResponse(ptr,RES_CODE::RES_CODE_OK,"");
}

std::string Service::getDatetime() {
  using namespace std::chrono;
  time_t t = system_clock::to_time_t(system_clock::now());
  struct tm *tm = localtime(&t);
  char buff[128]{0};
  sprintf(buff,"%d-%d-%d %d:%d:%d",tm->tm_year+1900,tm->tm_min+1,tm->tm_mday,tm->tm_hour,tm->tm_min,tm->tm_sec);
